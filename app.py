...
TABLES_2026: dict = {
    ...
    "ultimo_aggiornamento": "03/02/2026",
    "istat_rivalutazione_pct": 1.4,
...
# Archivio 2025 – per confronto nell'expander (Circ. INPS n. 25 del 29/01/2025, ISTAT +0,8%)
TABLES_2025: dict = {
    "anno": 2025,
    "fonte": "Circ. INPS n. 25 del 29/01/2025",
    "istat_rivalutazione_pct": 0.8,
    "contributi": {
        "TI": {
            "F1": {"totale": 1.68, "lav": 0.43},
            "F2": {"totale": 1.89, "lav": 0.48},
            "F3": {"totale": 2.31, "lav": 0.58},
            "F4": {"totale": 1.22, "lav": 0.31},
        },
        "TD": {
            "F1": {"totale": 1.79, "lav": 0.43},
            "F2": {"totale": 2.02, "lav": 0.48},
            "F3": {"totale": 2.46, "lav": 0.58},
            "F4": {"totale": 1.31, "lav": 0.31},
        },
    },
    "soglie_fasce": {"fascia1_max": 9.47, "fascia2_max": 11.54},
    "cassacolf": {"totale": 0.06, "lav": 0.02},
}
...
# ─────────────────────────────────────────────
# GENERAZIONE PDF – ATTESTAZIONE ANNUALE
# Equivalente informale della Certificazione Unica
# Il datore di lavoro domestico NON è sostituto d'imposta:
# emette questa attestazione per consentire al lavoratore
# di presentare il modello 730 / REDDITI.
# ─────────────────────────────────────────────
def genera_pdf_attestazione_cu(
    anno_rif: int,
    nome_lavoratore: str,
    cf_lavoratore: str,
    nome_datore: str,
    cf_datore: str,
    indirizzo_datore: str,
    data_inizio: str,
    data_fine: str,
    livello_ccnl: str,
    tipo_contratto: str,
    mesi_lavorati: int,
    lordo_annuo: float,
    inps_lav_annuo: float,
    cassacolf_lav_annuo: float,
    inps_totale_annuo: float,
    cassacolf_totale_annuo: float,
    netto_annuo: float,
    t: dict,
) -> bytes:
    """Genera l'attestazione annuale equivalente alla Certificazione Unica."""
    from reportlab.lib.pagesizes import A4
    from reportlab.lib import colors
    from reportlab.lib.units import mm
    from reportlab.platypus import (
    ...
    doc = SimpleDocTemplate(
        ...
        leftMargin=MARGIN, rightMargin=MARGIN,
        topMargin=14 * mm, bottomMargin=18 * mm,
        title=f"Attestazione Annuale {anno_rif} – {nome_lavoratore}",
        author="Gaetani & Partners Consulting",
        subject="Attestazione redditi lavoro domestico",
    )
    ...
    S = {
        "firm":   sty("cu_firm",  fontName="Helvetica-Bold", fontSize=14, textColor=WHITE,
                      alignment=TA_LEFT, leading=17),
        "hdr_r":  sty("cu_hdr_r", fontName="Helvetica",      fontSize=7.5,
                      textColor=GOLD_LITE, alignment=TA_RIGHT, leading=11),
        "title":  sty("cu_title", fontName="Helvetica-Bold", fontSize=13, textColor=NAVY,
                      alignment=TA_CENTER, leading=16, spaceBefore=4, spaceAfter=2),
        "sub":    sty("cu_sub",   fontName="Helvetica",      fontSize=8.5, textColor=MID_TXT,
                      alignment=TA_CENTER, leading=12, spaceAfter=6),
        "warn":   sty("cu_warn",  fontName="Helvetica-Oblique", fontSize=7.5, textColor=MID_TXT,
                      alignment=TA_CENTER, leading=11, spaceAfter=4),
        "sect":   sty("cu_sect",  fontName="Helvetica-Bold", fontSize=8, textColor=NAVY, leading=10),
        "il":     sty("cu_il",    fontName="Helvetica-Bold", fontSize=7.5, textColor=MID_TXT, leading=10),
        "iv":     sty("cu_iv",    fontName="Helvetica",      fontSize=8, textColor=DARK_TXT, leading=10),
        "lbl":    sty("cu_lbl",   fontName="Helvetica",      fontSize=9, textColor=DARK_TXT, leading=11),
        "lbl_i":  sty("cu_lbl_i", fontName="Helvetica",      fontSize=9, textColor=MID_TXT,
                      leading=11, leftIndent=10),
        "val":    sty("cu_val",   fontName="Helvetica",      fontSize=9, textColor=DARK_TXT,
                      alignment=TA_RIGHT, leading=11),
        "tot_l":  sty("cu_tot_l", fontName="Helvetica-Bold", fontSize=10, textColor=NAVY, leading=12),
        "tot_v":  sty("cu_tot_v", fontName="Helvetica-Bold", fontSize=10, textColor=NAVY,
                      alignment=TA_RIGHT, leading=12),
        "net_l":  sty("cu_net_l", fontName="Helvetica-Bold", fontSize=13, textColor=GOLD_LITE, leading=16),
        "net_v":  sty("cu_net_v", fontName="Helvetica-Bold", fontSize=13, textColor=GOLD_LITE,
                      alignment=TA_RIGHT, leading=16),
        "note":   sty("cu_note",  fontName="Helvetica-Oblique", fontSize=7, textColor=LITE_TXT, leading=9),
        "footer": sty("cu_foot",  fontName="Helvetica", fontSize=6.5, textColor=LITE_TXT,
                      alignment=TA_CENTER, leading=9),
        "firma":  sty("cu_firma", fontName="Helvetica", fontSize=8, textColor=MID_TXT,
                      alignment=TA_CENTER, leading=11),
    }
    ...
    hdr = Table([[
        Paragraph("Gaetani &amp; Partners Consulting", S["firm"]),
        Paragraph(
            f"Attestazione Annuale – Anno {anno_rif}<br/>"
            f"<font size='6.5' color='#AABBCC'>Emessa il {oggi_str}</font>",
            S["hdr_r"],
        ),
    ]], colWidths=[COL_W * 0.58, COL_W * 0.42])
    hdr.setStyle(TableStyle([
    ...
    # ── TITOLO ──
    story.append(Paragraph(f"ATTESTAZIONE REDDITI LAVORO DOMESTICO – ANNO {anno_rif}", S["title"]))
    story.append(Paragraph(
        "Documento rilasciato ai sensi del CCNL Lavoro Domestico.",
        S["sub"],
    ))
    story.append(Paragraph(
        "Il datore di lavoro domestico non riveste la qualifica di sostituto d'imposta (art. 23 DPR 600/1973). "
        "Il lavoratore è tenuto a dichiarare autonomamente i redditi percepiti (mod. 730 o REDDITI PF).",
        S["warn"],
    ))
    story.append(Spacer(1, 3 * mm))

    # ── DATI DATORE / LAVORATORE ──
    cw = COL_W / 4
    info_d = [
        [Paragraph("DATORE DI LAVORO", S["il"]), Paragraph(nome_datore or "—", S["iv"]),
         Paragraph("LAVORATORE / LAVORATRICE", S["il"]), Paragraph(nome_lavoratore or "—", S["iv"])],
        [Paragraph("CODICE FISCALE DATORE", S["il"]), Paragraph(cf_datore or "—", S["iv"]),
         Paragraph("CODICE FISCALE LAVORATORE", S["il"]), Paragraph(cf_lavoratore or "—", S["iv"])],
        [Paragraph("INDIRIZZO DATORE", S["il"]), Paragraph(indirizzo_datore or "—", S["iv"]),
         Paragraph("LIVELLO CCNL / CONTRATTO", S["il"]),
         Paragraph(f"{t['descrizione_livelli'].get(livello_ccnl, livello_ccnl)} · "
                   f"{'T. Indeterminato' if tipo_contratto == 'TI' else 'T. Determinato'}", S["iv"])],
        [Paragraph("PERIODO DI LAVORO", S["il"]),
         Paragraph(f"Dal {data_inizio or '—'} al {data_fine or '—'} ({mesi_lavorati} mesi)", S["iv"]),
         Paragraph("ANNO DI RIFERIMENTO", S["il"]), Paragraph(str(anno_rif), S["iv"])],
    ]
    info_tbl = Table(info_d, colWidths=[cw * 0.82, cw * 1.18, cw * 0.82, cw * 1.18])
    info_tbl.setStyle(TableStyle([
        ("BACKGROUND",   (0, 0), (-1, -1), CREAM),
        ("BACKGROUND",   (0, 0), (0, -1), colors.HexColor("#EEE9DA")),
        ("BACKGROUND",   (2, 0), (2, -1), colors.HexColor("#EEE9DA")),
    ...
    # ── DATI REDDITUALI ──
    imponibile_irpef = lordo_annuo - inps_lav_annuo - cassacolf_lav_annuo

    rows_red = [
        ("sect", "REDDITI DA LAVORO DOMESTICO – ANNO " + str(anno_rif), ""),
        ("ind",  "Retribuzione lorda annua percepita", euro(lordo_annuo)),
        ("ind",  "Contributi INPS trattenuti (quota lavoratore)", f"– {euro(inps_lav_annuo)}"),
        ("ind",  "CAS.SA.COLF trattenuta (quota lavoratore)", f"– {euro(cassacolf_lav_annuo)}"),
        ("tot",  "IMPONIBILE IRPEF ANNUO (lordo – contributi lav.)", euro(imponibile_irpef)),
        ("norm", "", ""),
        ("sect", "CONTRIBUTI INPS COMPLESSIVI VERSATI", ""),
        ("ind",  "INPS quota lavoratore (deducibile da reddito complessivo)", euro(inps_lav_annuo)),
        ("ind",  "INPS quota datore", euro(inps_totale_annuo - inps_lav_annuo)),
        ("tot",  "INPS totale versato (datore + lavoratore)", euro(inps_totale_annuo)),
        ("ind",  "CAS.SA.COLF quota lavoratore", euro(cassacolf_lav_annuo)),
        ("ind",  "CAS.SA.COLF quota datore", euro(cassacolf_totale_annuo - cassacolf_lav_annuo)),
        ("tot",  "CAS.SA.COLF totale versata", euro(cassacolf_totale_annuo)),
    ]
    ...
    for i, (rt, lab, val) in enumerate(rows_red):
        if rt == "sect":
            data_r.append([Paragraph(lab, S["sect"]), ""])
            style_r += [("BACKGROUND",(0,i),(-1,i),SECT_BG),("SPAN",(0,i),(-1,i)),
                        ("TOPPADDING",(0,i),(-1,i),3),("BOTTOMPADDING",(0,i),(-1,i),3)]
        elif rt == "tot":
            data_r.append([Paragraph(lab, S["tot_l"]), Paragraph(val, S["tot_v"])])
            style_r += [("BACKGROUND",(0,i),(-1,i),TOTAL_BG),("LINEABOVE",(0,i),(-1,i),1.2,GOLD)]
        elif rt == "norm":
            data_r.append(["", ""])
            style_r += [("BACKGROUND",(0,i),(-1,i),WHITE),("GRID",(0,i),(-1,i),0,WHITE),
                        ("TOPPADDING",(0,i),(-1,i),2),("BOTTOMPADDING",(0,i),(-1,i),2)]
        else:
            data_r.append([Paragraph(lab, S["lbl_i"]), Paragraph(val, S["val"])])

    red_tbl = Table(data_r, colWidths=[COL_W * 0.67, COL_W * 0.33])
    red_tbl.setStyle(TableStyle(style_r))
    story.append(red_tbl)
    story.append(Spacer(1, 4 * mm))

    # ── NETTO ANNUO ──
    netto_data = [[
        Paragraph("RETRIBUZIONE NETTA ANNUA EROGATA", S["net_l"]),
        Paragraph(euro(netto_annuo), S["net_v"]),
    ]]
    netto_tbl = Table(netto_data, colWidths=[COL_W * 0.62, COL_W * 0.38])
    netto_tbl.setStyle(TableStyle([
        ("BACKGROUND",   (0, 0), (-1, -1), NAVY),
        ("LINEABOVE",    (0, 0), (-1, 0), 2.5, GOLD),
        ("LINEBELOW",    (0, 0), (-1, 0), 2.5, GOLD),
    ...
    istr_data = [[
        Paragraph("ISTRUZIONI PER LA DICHIARAZIONE DEI REDDITI DEL LAVORATORE", S["sect"]),
    ], [
        Paragraph(
            "1. <b>Reddito da dichiarare</b>: indicare nel modello 730/REDDITI PF il reddito da lavoro "
            "dipendente pari all'imponibile IRPEF riportato sopra.<br/>"
            "2. <b>Deduzione contributi INPS</b>: i contributi a carico del lavoratore sono deducibili "
            "dal reddito complessivo (art. 10 TUIR); indicare l'importo nella casella dedicata.<br/>"
            "3. <b>CAS.SA.COLF</b>: la quota a carico del lavoratore è anch'essa deducibile.<br/>"
            "4. <b>Irpef</b>: sarà calcolata dal modello 730 o liquidata autonomamente tramite F24.",
            sty("cu_istr", fontName="Helvetica", fontSize=7.5, textColor=DARK_TXT,
                leading=11, leftIndent=4),
        ),
    ]]
    istr_tbl = Table(istr_data, colWidths=[COL_W])
    istr_tbl.setStyle(TableStyle([
        ("BACKGROUND",   (0, 0), (-1, 0), SECT_BG),
    ...
    firme = Table([[
        Paragraph(
            "Il datore di lavoro dichiara che i dati riportati<br/>"
            "corrispondono alle retribuzioni effettivamente erogate.<br/><br/><br/>"
            "_______________________________<br/>"
            f"<font size='7'>{nome_datore or ''} – {oggi_str}</font>",
            S["firma"],
        ),
        Paragraph(
            "Firma del lavoratore per ricevuta<br/><br/><br/><br/>"
    ...
    story.append(Paragraph(
        f"© {datetime.date.today().year} Gaetani &amp; Partners Consulting · "
        "Consulenza Tributaria &amp; Pianificazione Fiscale · "
        f"Fonte normativa: CCNL Lavoro Domestico · Circ. INPS n. 9/{anno_rif}",
        S["footer"],
    ))
...
def main():
    ...
    # ══════════════════════════════════════════════
    st.markdown("<div class='gp-section'>", unsafe_allow_html=True)
    st.markdown("<div class='gp-section-title'>2 · Retribuzione</div>", unsafe_allow_html=True)
    ...
    if convivente:
        ...
        st.markdown(
            ...
            f"<strong>{euro(min_h)}/h</strong> · mensile equivalente (× ore mese): "
            f"<strong>{euro(min_h * float(ore_mese))}</strong></div>", unsafe_allow_html=True
        )

    mode = st.radio("Modalità", [
        "Paga oraria LORDA",
        "Retribuzione MENSILE lorda",
        "Calcola lordo da NETTA oraria desiderata",
    ], horizontal=True)

    cp1, cp2 = st.columns([1, 1])
    with cp1:
        if mode == "Paga oraria LORDA":
        ...
        elif mode == "Retribuzione MENSILE lorda":
            default_mensile = round(float(max(min_h, 7.01)) * float(ore_mese), 2)
            paga_mensile_input = st.number_input(
                "Retribuzione mensile lorda (€)", min_value=0.0,
                value=float(max(min_m, default_mensile)) if convivente else default_mensile,
                step=1.0, format="%.2f",
            )
            paga_lorda = round(paga_mensile_input / float(ore_mese), 4) if ore_mese > 0 else 0.0
            paga_net_target = None
            st.markdown(
                f"<div class='gp-info' style='font-size:0.78rem;'>≡ Paga oraria derivata: "
                f"<strong>{euro(paga_lorda)}/h</strong> ({euro(paga_mensile_input)} ÷ {ore_mese:.0f}h)</div>",
                unsafe_allow_html=True,
            )
        else:
    ...
    # ══════════════════════════════════════════════
    # ══════════════════════════════════════════════
    # SEZ 9 – ATTESTAZIONE ANNUALE (equivalente CU)
    # ══════════════════════════════════════════════
    st.markdown("<div class='gp-section'>", unsafe_allow_html=True)
    st.markdown("<div class='gp-section-title'>9 · Attestazione annuale – equivalente Certificazione Unica</div>",
                unsafe_allow_html=True)

    st.markdown("""
    <div class="gp-info">
    ℹ️ Il datore di lavoro domestico <strong>non è sostituto d'imposta</strong> (art. 23 DPR 600/1973)
    e non emette la Certificazione Unica in senso tecnico. Deve però rilasciare al lavoratore
    questa <strong>attestazione annuale dei redditi erogati e dei contributi versati</strong>,
    necessaria per la compilazione del modello 730 o REDDITI PF.
    I valori annuali sono pre-compilati come stima (mese corrente × 12): verificare e correggere
    con i dati effettivi prima di generare il documento.
    </div>
    """, unsafe_allow_html=True)

    cu_a1, cu_a2, cu_a3 = st.columns(3)
    with cu_a1:
        cu_anno = st.number_input("Anno di riferimento", min_value=2020, max_value=2100,
                                  value=inp.anno, step=1, key="cu_anno")
        cu_mesi = st.number_input("Mesi lavorati nell'anno", min_value=1, max_value=12,
                                  value=12, step=1, key="cu_mesi")
    with cu_a2:
        cu_cf_lav = st.text_input("C.F. lavoratore", placeholder="RSSMRA80A01H501Z",
                                  key="cu_cf_lav").upper()
        cu_cf_dat = st.text_input("C.F. / P.IVA datore", placeholder="BNCMRI60B10F205K",
                                  key="cu_cf_dat").upper()
    with cu_a3:
        cu_indirizzo = st.text_input("Indirizzo datore", placeholder="Via Roma 1, 00100 Roma",
                                     key="cu_indirizzo")
        cu_data_inizio = st.text_input("Data inizio rapporto", placeholder="01/01/2026",
                                       key="cu_data_inizio")
        cu_data_fine   = st.text_input("Data fine rapporto (lasciare vuoto se in corso)",
                                       placeholder="", key="cu_data_fine")

    st.markdown("**Dati economici annuali** *(stima automatica × mesi – modificare con valori reali)*")
    cu_e1, cu_e2, cu_e3 = st.columns(3)

    stima_lordo   = round(res.lordo_totale * cu_mesi, 2)
    stima_inps_l  = round(res.inps_lav    * cu_mesi, 2)
    stima_casa_l  = round(res.cassa_lav   * cu_mesi, 2)
    stima_inps_t  = round((res.inps_lav + res.inps_datore) * cu_mesi, 2)
    stima_casa_t  = round((res.cassa_lav + res.cassa_datore) * cu_mesi, 2)
    stima_netto   = round(res.netto * cu_mesi, 2)

    with cu_e1:
        cu_lordo = st.number_input("Retribuzione lorda annua (€)", min_value=0.0,
                                   value=stima_lordo, step=1.0, format="%.2f", key="cu_lordo")
        cu_netto = st.number_input("Retribuzione netta annua erogata (€)", min_value=0.0,
                                   value=stima_netto, step=1.0, format="%.2f", key="cu_netto")
    with cu_e2:
        cu_inps_lav = st.number_input("INPS quota lavoratore annua (€)", min_value=0.0,
                                      value=stima_inps_l, step=1.0, format="%.2f", key="cu_inps_l")
        cu_inps_tot = st.number_input("INPS totale versato annuo (€)", min_value=0.0,
                                      value=stima_inps_t, step=1.0, format="%.2f", key="cu_inps_t")
    with cu_e3:
        cu_casa_lav = st.number_input("CAS.SA.COLF quota lavoratore annua (€)", min_value=0.0,
                                      value=stima_casa_l, step=1.0, format="%.2f", key="cu_casa_l")
        cu_casa_tot = st.number_input("CAS.SA.COLF totale versata annua (€)", min_value=0.0,
                                      value=stima_casa_t, step=1.0, format="%.2f", key="cu_casa_t")

    nome_file_cu = (
        f"attestazione_annuale_{cu_anno}"
        f"{'_' + nome_lav.replace(' ','_').lower() if nome_lav else ''}.pdf"
    )
    pdf_cu = genera_pdf_attestazione_cu(
        anno_rif=int(cu_anno),
        nome_lavoratore=nome_lav,
        cf_lavoratore=cu_cf_lav,
        nome_datore=nome_dat,
        cf_datore=cu_cf_dat,
        indirizzo_datore=cu_indirizzo,
        data_inizio=cu_data_inizio,
        data_fine=cu_data_fine,
        livello_ccnl=inp.livello_ccnl,
        tipo_contratto=inp.tipo_contratto,
        mesi_lavorati=int(cu_mesi),
        lordo_annuo=cu_lordo,
        inps_lav_annuo=cu_inps_lav,
        cassacolf_lav_annuo=cu_casa_lav,
        inps_totale_annuo=cu_inps_tot,
        cassacolf_totale_annuo=cu_casa_tot,
        netto_annuo=cu_netto,
        t=t,
    )
    st.download_button(
        label="⬇️  Genera Attestazione Annuale PDF",
        data=pdf_cu,
        file_name=nome_file_cu,
        mime="application/pdf",
        use_container_width=True,
        type="primary",
        help="Documento da rilasciare al lavoratore per la dichiarazione dei redditi (730/REDDITI PF)",
    )
    st.markdown("</div>", unsafe_allow_html=True)

    # ══════════════════════════════════════════════
    # SEZ 10 – PARAMETRI NORMATIVI (AVANZATO)
    # ══════════════════════════════════════════════
    with st.expander(f"⚙️  Tabelle normative {t['anno']} – sola lettura · Aggiornate al {t['ultimo_aggiornamento']}"):
        st.markdown(
            f"<div class='gp-info'>"
            f"Tabelle aggiornate alla <strong>{t['fonte']}</strong> · "
            f"Rivalutazione ISTAT: <strong>+{t['istat_rivalutazione_pct']}%</strong> · "
            f"Aggiornamento annuale: ogni febbraio con la nuova circolare INPS. "
            f"Prossimo aggiornamento atteso: <strong>febbraio 2027</strong>."
            f"</div>",
            unsafe_allow_html=True,
        )

        st.markdown(f"**Contributi INPS orari {t['anno']} – confronto con {TABLES_2025['anno']}**")
        cti, ctd = t["contributi"]["TI"], t["contributi"]["TD"]
        t25_ti, t25_td = TABLES_2025["contributi"]["TI"], TABLES_2025["contributi"]["TD"]
        defs = [
            ("F1", f"≤ €{t['soglie_fasce']['fascia1_max']:.2f}/h eff. (2026) | ≤ €{TABLES_2025['soglie_fasce']['fascia1_max']:.2f} (2025)"),
            ("F2", f"> €{t['soglie_fasce']['fascia1_max']:.2f} fino a €{t['soglie_fasce']['fascia2_max']:.2f}/h"),
            ("F3", f"> €{t['soglie_fasce']['fascia2_max']:.2f}/h"),
        ...
        for cod, cond in defs:
            ...
            ti5, td5 = t25_ti[cod], t25_td[cod]
            contr_rows += (
                f"<tr><td><strong>{cod}</strong></td><td style='font-size:0.75rem;'>{cond}</td>"
                f"<td class='amt'>€{ti['totale']:.2f}</td>"
                f"<td class='amt' style='color:#7A4F00;font-size:0.8rem;'>{delta(ti['totale'], ti5['totale'])}</td>"
                f"<td class='amt'>€{ti['lav']:.2f}</td>"
                f"<td class='amt'>€{td['totale']:.2f}</td>"
                f"<td class='amt' style='color:#7A4F00;font-size:0.8rem;'>{delta(td['totale'], td5['totale'])}</td>"
                f"<td class='amt'>€{td['lav']:.2f}</td>"
                "</tr>"
            )
        st.markdown(
            f"<table class='ced-table'>"
            f"<thead><tr><th>Fascia</th><th>Condizione</th>"
            f"<th>TI Tot.</th><th>Δ '25</th><th>TI Lav.</th>"
            f"<th>TD Tot.</th><th>Δ '25</th><th>TD Lav.</th></tr></thead>"
            f"<tbody>{contr_rows}</tbody>"
            f"<tfoot><tr><td colspan='8' style='padding:0.5rem 1rem;color:#666;font-size:0.74rem;'>"
            f"CAS.SA.COLF (invariata): totale €0,06/h · lavoratore €0,02/h · datore €0,04/h"
            "</td></tr></tfoot></table>",
            unsafe_allow_html=True,
        )

        st.markdown("**Minimi retributivi CCNL – non conviventi (€/ora) e conviventi (€/mese)**")
        min_rows = "".join(
        ...
        st.markdown(
            ...
            f"<th style='text-align:right'>Min. €/h</th>"
            f"<th style='text-align:right'>Convivente €/mese</th>"
